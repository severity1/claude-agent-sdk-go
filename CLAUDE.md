...
- **Cyclomatic complexity**: Keep functions under complexity 15 (measured by gocyclo); use `//nolint:gocyclo` on functions that legitimately exceed the threshold (large table-driven test functions, JSONRPC dispatch switches like `routeMcpMethod`); higher complexity acceptable for table-driven tests, examples, orchestration code, method dispatch
