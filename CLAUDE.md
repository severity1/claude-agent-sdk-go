...
- **Process cleanup**: SIGTERM -> wait 5 seconds -> SIGKILL; `terminateProcess()` checks `stdoutDone` first and returns early (nil) if the CLI already exited, avoiding a redundant Signal/Kill; calls `t.cmd.Wait()` on the stdoutDone fast path to reap the process and release the exec.Cmd watchCtx goroutine (mirrors Python `subprocess_cli.py:568`; expected signal exits are not surfaced as errors); `isProcessAlreadyFinishedError()` suppresses expected termination errors including Windows `"TerminateProcess: Access is denied"` (returned when TerminateProcess targets an already-exited process)
...
- **TestMain cross-platform mock CLI**: `internal/subprocess/mock_cli_test.go` and `internal/cli/mock_cli_test.go` each define a `TestMain` that checks a package-scoped env var (`CLAUDE_SDK_TEST_MOCK_MODE` / `CLAUDE_SDK_TEST_CLI_MOCK_MODE`) and re-enters the compiled test binary as a mock CLI process before `m.Run()`. This is the Go analog of Python's `sys.executable -c "..."` and matches the `os/exec` stdlib `TestHelperProcess` idiom. Replaces per-platform `.bat`/`.sh` fixtures. Subprocess mock modes: `default`, `long_running`, `should_fail`, `check_environment`, `invalid_output`, `with_control_protocol`, `with_stderr`, `init_error` (`init_error`: emits PID on stderr, responds to initialize with `subtype:"error"`, then keeps reading stdin so the SDK must terminate the process - used by `TestTransportConnectFailureTearsDown`). Test helpers (e.g. `newTransportMockCLI(t)`, `newTransportMockCLIInitError(t)`) set the env var via `t.Setenv` (auto-cleaned). `t.Setenv` is incompatible with `t.Parallel()` at the same scope - keep test functions using these helpers sequential. Race-mode parent contexts need ~30s budget because spawning the Go test binary is slower than bash.
...
- **Connect failure cleanup invariant**: `subprocess.Transport.Connect()` calls `teardownLocked()` when `setupControlProtocol` fails before returning the error. `teardownLocked()` is the unified post-Start cleanup primitive: cancels context, closes the control protocol, closes stdin, waits for goroutines, terminates the process, and runs the cleanup func. Shared between `Close()` (called after setting `connected=false`) and the Connect error path, so no subprocess is ever leaked on a failed connect. `TestTransportConnectFailureTearsDown` verifies this by capturing the mock CLI PID via `StderrCallback` and probing `syscall.Signal(0)` after the failed `Connect()` returns.
...
- Recent focus: Phase1 item #6 - agents via initialize + unified streaming (Python PR #468): `subprocess.New()` single constructor; both `Query()` and `Client` use streaming mode + unconditional initialize handshake; agents travel on initialize control request; `EndInput()` closes stdin write side; post-feat fixes: (1) `InitializeRequest.Hooks` emits `"hooks":null` (no omitempty, matching Python wire shape), (2) `StreamMessage.SessionID`/`ParentToolUseID` lack `omitempty` (wire always emits both keys), (3) `HandleControlInitErr()` is no-op after successful Initialize (race guard for long-lived clients), (4) cross-platform mock CLI via `TestMain` + `os.Args[0]` re-entrancy replaces `.bat`/`.sh` fixtures in both `internal/subprocess` and `internal/cli`, (5) `examples/15_programmatic_subagents` Example 4 adds large-payload agent smoke test (~260 KB via initialize, bypasses ARG_MAX); parity-review follow-up fixes: (6) `Transport.EndInput(ctx)` promoted to public `Transport` interface (was only on `subprocess.Transport`, found via inline-interface assertion in `query.go`), (7) `teardownLocked()` extracted as unified post-Start cleanup primitive shared between `Close()` and `Connect()` error path so no subprocess leaks on failed connect, (8) `terminateProcess()` calls `cmd.Wait()` on stdoutDone fast path to reap process and release exec.Cmd watchCtx goroutine, (9) `init_error` mock mode + `TestTransportConnectFailureTearsDown` test verifies subprocess is reaped after connect failure (probes PID via `syscall.Signal(0)`); Phase1 item #5 - MCP tool annotations (Python PR #551): `shared.ToolAnnotations` struct (authoring side) + `ToolOption`/`WithToolAnnotations` on `NewTool()`; `annotationsToMap` json round-trip; `buildToolsListResult` extracted from `routeMcpMethod` (gocyclo refactor, removed nolint); Phase1 item #4 - 3 new hook events + missing fields (Python PR #545): Notification, SubagentStart, PermissionRequest event types; hook count 7 -> 10
