...
| 1 | #516 | get_mcp_status() method | Jan 26 | feat | done | #124 | Python added get_mcp_status() returning McpStatusResponse with list of McpServerStatus (name, status, serverInfo, error, config, scope, tools). Add GetMcpStatus(ctx) to Client interface, control request subtype "get_mcp_status", McpServerStatus/McpStatusResponse types in `internal/control/types.go`, wire through Transport. |
