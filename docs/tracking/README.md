...
| 3 | #506 | Properly populate AssistantMessage error field | Feb 3 | fix | done | feature/phase1-1 | Fixed parser to read `error` from top-level `data["error"]` (was incorrectly reading from nested `data["message"]["error"]`). Updated tests to use correct wire format. Added billing_error and server_error test cases. Regression test verifies nested error is ignored. |
| 4 | #545 | Missing hook events + fix fields | Feb 3 | feat | pending | - | Python added 3 hook events (`Notification`, `SubagentStart`, `PermissionRequest`) each with HookInput + HookSpecificOutput types. Also fixed missing fields on existing hooks: `tool_use_id` on PreToolUseHookInput and PostToolUseHookInput; `agent_id`/`agent_transcript_path`/`agent_type` as flat required fields on SubagentStopHookInput (NOT via a mixin — the mixin lands separately in item #13 / PR #628); `additionalContext` on PreToolUseHookSpecificOutput; `updatedMCPToolOutput` on PostToolUseHookSpecificOutput. Add 3 HookEvent constants, 3 HookInput structs, 3 HookSpecificOutput structs in `internal/control/types_hook.go`; add `tool_use_id` to PreToolUse/PostToolUse inputs; add the 3 agent fields directly to SubagentStopHookInput; add the new output fields. |
