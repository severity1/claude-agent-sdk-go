...
| 10 | #620 | MCP control methods + typed McpServerStatus | Mar 3 | feat | pending | - | Partial pre-impl in Phase 1 #1 (#117): `McpServerConnectionStatus` enum and `McpServerInfo` struct are already in `internal/control/types.go` and re-exported in `types.go`. Remaining work: add `ReconnectMcpServer(ctx, name)` and `ToggleMcpServer(ctx, name, enabled)` on Client interface with new control request subtypes, wire through Transport. |
...
| 13 | #628 | agent_id/agent_type in hook inputs | Mar 3 | feat | done | #117 | Completed as part of Phase 1 item #4 (Python PR #545). `AgentID *string` and `AgentType *string` are present on `PreToolUseHookInput`, `PostToolUseHookInput`, and `PostToolUseFailureHookInput` in `internal/control/types_hook.go`. No additional work needed. |
